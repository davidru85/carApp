...
> **Project status:** **Phase 1 open.** `E1-01` through `E1-04`, `E3-06`, `E3-01` and `E0-07` have delivered the SQLDelight database, the complete local Vehicle repository, the Fuel Entry domain, executable provider decoupling, reviewed Firestore rules and the native walking skeleton. `E1-05`, the human-gated consumption calculation story, is next. The project builds on both platforms and `main` is protected by nine required CI checks.
...
| 1 - Local persistence **(active)** | `E1-01` through `E1-04` and the `E0-07` gate are complete; `E1-05` through `E1-10` remain | The walking skeleton already proves the native local/remote path. E1-05 is the human-reviewed consumption gate |
