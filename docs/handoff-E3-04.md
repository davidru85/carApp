...
- CI on the correction round: run `35588801389` at `b63f2e7` reports **all ten required checks green**. `shared-tests` stalled and failed on the first two attempts of that run and passed on the third, which is the same pre-existing environment stall the Verification Run section analyses: the correction round does not touch the module that stalls (`:core:sync` completed its 101 tests in the failing logs) and the same local step passes five consecutive cold-daemon, no-cache runs in 19 s each.
