...
| **Firebase** | Firebase BOM, GitLive Firebase SDK, Firebase Apple SDK, GoogleSignIn-iOS, Google Services plugin | GitLive 2.6.x wraps platform-specific native Firebase SDKs. Its Apple cinterop bindings require the exact Firebase Apple version they were generated against; GoogleSignIn-iOS shares Firebase Apple transitive packages. GitLive, Firebase Apple and GoogleSignIn-iOS require graph revalidation when any member moves. GitLive 3.0 alpha is out of scope. |
| **Android native authentication** | Credential Manager, Credential Manager Play Services Auth, Google ID | Credential Manager core and its Play Services bridge share one version line; Google ID supplies the Google credential type consumed by that line. All three artifacts move only after the complete Android provider flow is revalidated. |
...
| **Cloud Functions and D-63 debt** | Node.js 22, Firebase Functions, Firebase Admin, `@google-cloud/billing`, TypeScript, `@types/node` | Node.js 22 is the current runtime that supports the D-63 1st gen Auth exception and all 2nd gen functions in one package. The runtime, trigger and expiring D-68 advisory review move together under TD-01. |
...
| **GitHub Actions runtime** | `actions/checkout`, `actions/setup-java`, `actions/setup-node`, `gradle/actions/setup-gradle` | These foundational actions are pinned to Node.js 24-compatible releases and immutable commit SHAs. Their versions move only through a human-reviewed CI maintenance change. |
...
| **Android Vehicle UI support** | Compose BOM, Navigation, AndroidX Test runner/rules, `compileSdk`, emulator API and emulator-runner action | Compose UI tests are BOM-managed, Navigation and AndroidX Test must compile against the pinned Android build, and the CI emulator must provide the app's target API. |
...
| Firebase Admin SDK | `firebase-admin` | 14.3.0 | [ADR-0067](adr/0067-contain-development-cloud-costs.md) (`D-66`), [ADR-0069](adr/0069-retain-official-functions-stack-with-unreachable-moderate-advisory.md) (`D-68`) | Mandatory non-optional peer of Firebase Functions 7.3.2 even before D-63 uses its Admin APIs. Its current moderate production transitive residual is dynamically unreachable from `stopBilling` and expires into TD-01 review. |
| Functions HTTP parser (transitive) | `qs` | 6.16.0 | [ADR-0131](adr/0131-resolve-qs-advisories-within-existing-ranges.md) (`D-130`) | Lockfile-only resolution inside the existing Express and Body Parser ranges; fixes GHSA-x5fp-wj9c-mxmx and GHSA-4mjr-xmp4-gh2g without adding a direct dependency. |
...
| GitHub Actions checkout | `actions/checkout` | 5.1.0 (`fbc6f3992d24b796d5a048ff273f7fcc4a7b6c09`) | `E3-03` | Node.js 24 runtime; immutable commit pin verified against the official `v5.1.0` tag. |
| GitHub Actions Java setup | `actions/setup-java` | 5.7.0 (`b6effb05e454b25005698d916606bdc6ffcbf961`) | `E3-03` | Node.js 24 runtime; immutable commit pin verified against the official `v5.7.0` tag. |
...
| GitHub Actions Gradle setup | `gradle/actions/setup-gradle` | 5.0.2 (`0723195856401067f7a2779048b490ace7a47d7c`) | `E3-03` | Node.js 24 runtime; immutable commit pin verified against the official `v5.0.2` tag; preserves the existing cache provider. |
...
| Android navigation | `androidx.navigation:navigation-compose` | 2.9.8 | [ADR-0085](adr/0085-use-compose-navigation-and-instrumented-ui-tests.md) (`D-84`) | Stable Navigation 2.x release verified against the official AndroidX release page on 2026-08-30; compiles with the pinned AGP 9.3.1, Kotlin 2.4.10 and Compose BOM 2026.08.00 group. |
| Android Compose UI tests | `androidx.compose.ui:ui-test-junit4`, `ui-test-manifest` | managed by Compose BOM 2026.08.00 | [ADR-0085](adr/0085-use-compose-navigation-and-instrumented-ui-tests.md) (`D-84`) | Instrumented test artifact plus debug manifest support; no separate Compose version pin. |
| AndroidX Test | `androidx.test:runner`, `androidx.test:rules` | 1.7.0 | [ADR-0085](adr/0085-use-compose-navigation-and-instrumented-ui-tests.md) (`D-84`) | Stable runner/rules pair verified against the official AndroidX Test release page on 2026-08-30. |
| Android background work | `androidx.work:work-runtime` | 2.11.2 | [ADR-0185](adr/0185-pin-workmanager-for-the-android-background-cadence.md) (`D-184`) | Supplies the `enqueueUniquePeriodicWork` cadence and `CoroutineWorker` that `docs/CONTRACTS.md §9.1` requires for the Android `Periodic` trigger. The newest stable release, not the 2.12.0-rc line; it compiles against the pinned `compileSdk` 37 and `minSdk` 26. |
| Android emulator CI action | `ReactiveCircus/android-emulator-runner` | 2.38.0 (`a421e43855164a8197daf9d8d40fe71c6996bb0d`) | [ADR-0085](adr/0085-use-compose-navigation-and-instrumented-ui-tests.md) (`D-84`) | Immutable peeled tag commit verified with `git ls-remote`; runs API 36 x86_64 with Linux KVM. |
...
| iOS Keychain acceptance target | XCUITest bundle identifier | `com.ruizurraca.carapp.uitests` | [ADR-0075](adr/0075-use-xcuitest-for-keychain-persistence-acceptance.md) (`D-74`) | Test-only and non-distributed; drives the signed Debug app as a separate process and is absent from Release archives. |
...
| Android credential acquisition | `androidx.credentials:credentials` | 1.6.0 | [ADR-0113](adr/0113-android-google-credential-acquisition.md) (`D-112`) | Stable Credential Manager line released 2026-04-08. Its minSdk 23 remains below the project's fixed minSdk 26. |
| Android credential Play Services bridge | `androidx.credentials:credentials-play-services-auth` | 1.6.0 | [ADR-0113](adr/0113-android-google-credential-acquisition.md) (`D-112`) | Moves with Credential Manager core in the Android native authentication compatibility set. |
| Android Google ID credential | `com.google.android.libraries.identity.googleid:googleid` | 1.2.0 | [ADR-0113](adr/0113-android-google-credential-acquisition.md) (`D-112`) | Stable Google credential implementation consumed by the selected Credential Manager line. |
...
| iOS Google credential acquisition | `GoogleSignIn-iOS` Swift package | 9.2.0 | [ADR-0114](adr/0114-ios-native-provider-acquisition.md) (`D-113`) | Exact pin, not a range and not the newer 10.0.0 line. It belongs to the Firebase compatibility set and MUST be revalidated whenever D-65 moves. |
