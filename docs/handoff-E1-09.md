...
- Implemented `@MainActor ObservableObject` screen models in `iosApp/ViewModels.swift`: `VehicleListViewModel`, `VehicleFormViewModel`, `VehicleDetailViewModel`, `FuelEntryFormViewModel`, and `DiagnosticsViewModel`. Each launches a single observation `Task` in `init`, cancels it in `deinit`, and invokes the corresponding keyed release method on `SwiftAppGraph` (`releaseVehicleFormStateHolder`, `releaseFuelEntryFormStateHolder`).
