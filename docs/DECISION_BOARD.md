...
| D-76 | Vehicle validation boundary | Pure Vehicle validators receive immutable pre-write facts; E1-03 loads those facts and invokes the validators before persistence | CRUD use cases with additional repository query ports; validation only inside the data repository | Accepted | Keeps E1-02 Kotlin-pure, makes normalisation and every validation branch independently testable, and avoids exposing persistence-shaped queries in the public repository contract. |
