...
| D-75 | Firebase standalone Kotlin/Native test exception | Exempt exactly `:integration:firebase-auth` and `:integration:firebase-firestore` from standalone `iosSimulatorArm64Test`; retain Android-host unit tests and real-host XCUITest coverage | Add CocoaPods beside SwiftPM; adopt experimental SwiftPM import on beta Kotlin; maintain test-only Firebase XCFrameworks | Accepted | GitLive does not link Apple SDK dependencies transitively into standalone Kotlin/Native test binaries. The exact-set CI guard prevents the exception from expanding silently, and TD-01 reviews its two upstream expiry signals. |
