...
| D-185 | iOS trigger-ban enforcement mechanism | A source rule bans `PostWriteDebounce`, `ConnectivityRecovered` and `Periodic` from `SyncStateHolder` call sites on the iOS platform boundary (Kotlin and Swift), accepting `AppForeground` and `PullToRefresh` | Accepted | The declared Konsist fixture could not cover the rule: Konsist parses Kotlin only while `§20.10` prohibits the calls from Swift UI code, and the ban must not reject the `syncController()` route `§9.1` requires. See ADR-0186. |
