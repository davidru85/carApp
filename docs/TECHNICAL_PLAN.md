...
| D-190 | `E1-18` deadlock fix | Take `io` off the test scheduler so the driver's blocking close cannot seize the thread the graph needs, and keep `graphScope` and the local observation on the confined `default` so the engine's timers stay on virtual time; supersedes the `D-189` stopgap | Accepted | Measured over 10 consecutive `provider-decoupling` runs: 0 hangs, against 1-in-10 before. Moving the sync engine's scope to a real `io` instead produced an 80% assertion-failure rate, because its `delay()` calls stopped being virtual; that is why the scope moved to `default`. `D-189`'s raised step limits are reverted. See ADR-0191. |
