...
| D-124 | Automatic anonymous retry gate | `LocalOwnerAdoption.acquireAnonymousUidIfWaiting()` requires the sentinel owner, a `SignedOut` auth state and at least one row still owned by the sentinel | Accepted | Uses the product data itself as the "started offline" signal, so no new column, migration or second source of truth is introduced; acquisition for a device that wrote nothing moves to its first write. |
| D-125 | Adoption read gate | `AdoptionGatedVehicleRepository` awaits adoption before every Vehicle read and write in the composition root | Accepted | Closes the window in which an authenticated owner's list resolves empty while the rows still belong to the sentinel; hosts, state holders and the exported UI contract are unchanged. |
...
Status: open; `E2-01`, `E2-02` and `E2-03` are complete, `E2-03` having merged on 2026-09-06 through pull request #54, and `E2-06` is in progress.
