...
| D-163 | Departure process-death residual risk | Accept the process-death window between a successful remote step and the completed local clear, record it in `docs/SECURITY.md`, and track the durable recovery marker as the separately scoped `E2-09` | Accepted | The honest options are to close the window properly in its own story or to state it plainly; silently claiming resumability was the actual defect. |
