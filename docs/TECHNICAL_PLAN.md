...
| D-139 | Expired anonymous token retry convergence | Permit well-formed expired anonymous ID token on retry if and only if Auth user is already deleted (`auth/user-not-found`) | Accepted | Preserves the `{ anonymousIdToken }` wire contract and enables self-healing §11.3 retry convergence after interruption even when > 1 hour has elapsed, without extra Firestore collections or write overhead. |
