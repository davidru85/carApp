...
| D-169 | Pull-cursor sub-millisecond truncation | Accept the millisecond cursor and bound the later-page progress guarantee to millisecond-distinguishable timestamp clusters | Accepted | The owner selected option A on 2026-09-13. The downward-truncated `>=` boundary loses no data; an oversized same-millisecond cluster fails closed with `SyncError.ConflictUnresolved`. See ADR-0170. |
...
- Pull overlap prevents silent cursor loss; `startAt(overlapSince)` includes every document at the first-page boundary. `startAfter` on the previous page's timestamp/document-ID cursor advances every millisecond-distinguishable cluster; an oversized same-millisecond cluster fails closed with `SyncError.ConflictUnresolved` (`D-169`).
