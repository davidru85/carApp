...
| D-180 | Guarding the two `AppGraph` surfaces | `contract-check` implements assertion 14 and adds assertion 34, parsing the `§20.10` block and the real declarations | Accepted | Kotlin default arguments never reach the generated header, and the Kotlin-facing `AppGraph` is hidden from Objective-C export, so both surfaces needed a source-level guard. See ADR-0181. |
