...
| D-143 | Account-deletion erasure of orphan-cleanup authorizations | Account deletion purges every orphan-cleanup authorization bound to the deleted UID that is visible to the purge when it runs; a concurrent issuance may still write one afterwards, and closing that race is D-149 / E3-15, so a missed record is cleaned up solely by the existing asynchronous Firestore TTL, with no proven maximum. Internal server-only collections are declared in a separate registry excluded from D-63. | Accepted |
...
| D-148 | Admin verification at ticket issuance | The ticket issuer resolves the caller's current Auth record before writing and issues only when the snapshot it observes exists, is known to be enabled and is still anonymous; an unavailable enabled state rejects. The guarantee is about the state observed at that lookup: a token already linked, disabled, deleted or state-unknown is rejected and creates no authorization, but eligibility is not proven to hold until the Firestore write commits — that window is D-150 / E3-16. | Accepted |
...
| D-150 | Issuance lookup-to-write window | Mechanism for the window between the D-148 Admin lookup and the Firestore authorization write, in which the account can be linked, disabled or deleted; no recommendation is offered and no option is pre-selected. A second Admin read, a post-write read-back, a retry or a compensating delete may not be described as closing it. Distinct from D-149, which owns the account-deletion race. | Pending |
| D-151 | Durable conversion marker storage | Two normalized schema v3 tables: a single-row `account_conversion_operation` holding the anonymous UID, the permanent UID, the cleanup ticket and a resume `phase`, plus `account_conversion_snapshot` keyed by `(entityType, entityId)` | Accepted |
...
| D-163 | Departure process-death residual risk (superseded by `D-165`) | Accept the process-death window between a successful remote step and the completed local clear, record it in `docs/SECURITY.md`, and track the durable recovery marker as the separately scoped `E2-09` | Superseded |
...
| D-172 | `AppGraph.close()` versus an in-flight sync cycle | `AppGraph.close()` releases the `DatabaseHandle` from `graphScope`'s completion handler, with a 5-second bounded backstop that releases it anyway, and completes every in-flight `sync()` awaiter with `PersistenceError.DatabaseUnavailable`; `close()` stays synchronous. | Accepted |
...
| D-174 | Provider timestamp precision in the pull cursor | The integration delivers the provider's full-precision ordering timestamp and the engine's in-cycle cursor carries it untruncated, rebuilding the later-page `startAfter` boundary from it; the persisted `sync_cursor` stays an epoch-millisecond cycle anchor the 30-second overlap re-includes. No local-schema change and `§9.6` LWW stays epoch milliseconds. | Accepted |
| D-175 | Silent `shared-tests` stall ownership | The unbounded `awaitState` collector cleanup and the eight bare `while (…) yield()` polls are fixed inside `E3-03` as correction round 17, in test code only; a separate test-infrastructure story and relaxing the CI check were rejected. | Accepted |
| D-176 | CI job hang guard sizing | The job `timeout-minutes` ceiling is 40 minutes so it stays well above the slowest measured healthy run; the limit kills a hung job and MUST NOT be used as a speed budget. The `E0-05` 20-minute whole-run objective is unchanged. | Accepted |
| D-177 | Graph-backed test time advancement | Every shared test that mounts an `AppGraph` advances virtual time with the bounded `advanceGraphWork` helper rather than `advanceUntilIdle()`, so a self-re-arming coroutine cannot kill the test task. Bounding the production `scheduleAdoptionRetry` loop is a separate production concern and is not discharged here. | Accepted |
| D-178 | `:wiring:firebase` product logic | Executed as a declaration-shape rule: functions and private properties are admitted, a Koin `Module` is admitted, and every type declaration or `expect`/`actual` is rejected at any visibility. | Accepted |
| D-179 | Integration types outside their modules | No module other than `:wiring:firebase` and `:integration:*` may mention the `com.ruizurraca.carapp.integration.` package in its source. | Accepted |
| D-180 | Guarding the two `AppGraph` surfaces | `contract-check` assertion 14 is implemented and assertions 34 and 35 are added, comparing the Kotlin-facing and Swift-facing `§20.10` code blocks with the real `AppGraph` and `SwiftAppGraph` member by member; `§20.10` gains the already-shipped `syncStateHolder(scope)`. | Accepted |
| D-181 | Real platform scheduling port | `DefaultAppGraph` asks the injected `SyncTriggerAdapter` to arrange the `Periodic` cadence once per graph, and each platform's worker or handler requests the cycle on the same in-process `SyncController`. | Accepted |
| D-182 | Process-scoped app graph | The `AppGraph` is built once per process by the application and shared by the UI and the background worker; the Activity releases only the state holders it created, and the graph is released by process death. | Accepted |
| D-183 | Foreground threshold in shared code | `SyncStateHolder.onForegroundReturn(backgroundMillis)` applies `FOREGROUND_RESUME_THRESHOLD_MS`; a host reports a monotonic background duration or `null` for a cold start, which is always a trigger. | Accepted |
...
| D-189 | Temporary CI step-timeout raise for the `E1-18` deadlock | The two stalling macOS steps rise to 15 minutes so `E3-12` can progress, while `E1-18` delivers the real fix: split `io` away from the test scheduler so the driver's blocking close runs on a free thread. The measure MUST be removed when `E1-18` lands. | Superseded |
| D-190 | `E1-18` deadlock fix | `io` is a real dispatcher in graph fixtures, `graphScope` and the vehicle list's local observation run on the scheduler-confined `default`, test database handles are released without blocking the caller, and the recovery window's closing republish discards a stale resolved-empty listing and re-reads it; supersedes `D-189`. | Accepted |
