...
| D-190 | `E1-18` deadlock fix | `io` is a real dispatcher in graph fixtures, `graphScope` and the vehicle list's local observation run on the scheduler-confined `default`, and the recovery window's closing republish re-reads over a stale resolved-empty listing; supersedes `D-189` | Accepted |
