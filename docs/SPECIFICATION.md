...
| D-180 | Guarding the two `AppGraph` surfaces | `contract-check` assertion 14 is implemented and assertion 34 is added, comparing the `§20.10` code block with the real `AppGraph` member by member; `§20.10` gains the already-shipped `syncStateHolder(scope)`. | Accepted |
