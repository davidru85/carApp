...
| D-169 | Pull-cursor sub-millisecond truncation | Option A is recommended: accept the millisecond-truncated cursor and bound the `§9.4` later-page progress guarantee to timestamp clusters distinguishable at millisecond resolution, documenting that a cluster larger than the page limit within one millisecond can fail the cycle with `SyncError.ConflictUnresolved`. Option B preserves provider precision but changes the `§20.7` epoch-millisecond cursor and the `§9.4`/`§9.6` arbitration contract. No option is selected; the owner chooses. | Proposed |
| D-170 | Malformed remote documents cannot be quarantined through `RemoteSyncSource` | Option A is recommended: `pullChanges` returns raw per-document results so the engine validates and quarantines, keeping `§9.5` classification beside the local transaction that writes the quarantine row. Option B returns a per-document quarantine verdict from the source but moves `§9.5` classification into the integration. Both change `RemotePage` / `§20.7`. Option C skips malformed documents and loses data silently. No option is selected; the owner chooses. | Proposed |
