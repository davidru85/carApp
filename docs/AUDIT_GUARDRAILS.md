...
| Implementation readiness | Ready for story intake | Every `F-xx` follow-up finding has been absorbed into the normative and derived docs. Story-specific Ready checks still apply. |
| Temporary file deletion | Ready after owner review | This file can be deleted after the owner confirms the audit closure summary is no longer needed. |
...
| `F-12` | High | Firestore contract | Closed | `docs/CONTRACTS.md §3`, `§16` | Remote payload validation said "every field", but local-only fields were excluded elsewhere. Extra-key policy was not exact. | The owner chose a closed remote schema. `docs/CONTRACTS.md §16` now defines the exact required key sets for remote `Vehicle` and `FuelEntry`, requires nullable fields to be present as explicit `null`, forbids unknown collections, extra keys and local-only metadata, fixes MVP remote `schemaVersion = 1`, and requires `deleted == (deletedAt != null)`. | Closed on 2026-08-17 by updating `AGENTS.md`, `docs/SPECIFICATION.md §10`, `docs/CONTRACTS.md §16`, `E3-01` backlog criteria, emulator-test requirements, this register and the project log. |
...
| `F-15` | Medium | Consumption contract | Closed | `docs/CONTRACTS.md §20.6`, `§20.4` | `ConsumptionInvalidReason.EndEntryNotFullTank` existed, but `ConsumptionReport.segments` is one per full-tank entry, so the reason had no clear producer. `FuelEntryListItem.invalidReason` for partial entries was undefined. | The owner chose to keep `EndEntryNotFullTank` for partial refuels. A partial row maps to `FuelEntryListItem(consumption = null, invalidReason = EndEntryNotFullTank)`, does not produce a `SegmentResult`, and still contributes its litres to the next full-to-full segment when it falls inside that segment. Full-tank rows map to valid segment consumption or the segment's invalid reason. | Closed on 2026-08-17 by updating `docs/SPECIFICATION.md §6 R-3`, `docs/CONTRACTS.md §4`, `§13`, `§20.4`, `§20.6`, `E1-05` / `E1-06` / `E1-08` / `E1-09` backlog criteria, this register and the project log. |
...
| :--- | :--- | :--- |
| Closed | `F-01`, `F-02`, `F-03`, `F-04`, `F-05`, `F-06`, `F-07`, `F-08`, `F-09`, `F-10`, `F-11`, `F-12`, `F-13`, `F-14`, `F-15`, `F-16`, `F-17`, `F-18`, `F-19` | Module ownership, Swift-facing ABI, shared state-holder declarations, contract-check allowlist, monetary persistence, supported currencies, name normalization, MVP fuel-type scope, stored sync state, local-owner adoption ordering, account deletion server/Admin execution, closed Firestore remote schemas, pull pagination, odometer recompute set, list-only partial-refuel consumption explanation, malformed-payload quarantine, settings reset semantics and crash-reporting module lists have been aligned. |
| None | — | No active follow-up findings remain in this audit register. |
