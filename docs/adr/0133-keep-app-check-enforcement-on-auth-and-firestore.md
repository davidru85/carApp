...
verifies the captured anonymous ID token before any deletion.

## Options Considered

| Option | Benefits | Costs / Risks |
|--------|----------|---------------|
| Keep D-67 unchanged: no App Check enforcement on Cloud Functions | No client or infrastructure change in this story; the callables still require a verified Firebase caller and a valid captured anonymous token; the accepted D-67 scope remains stable. | Any authenticated client can invoke the callables without an App Check token; abuse mitigation stays with authentication checks and the D-66 billing cutoff. |
...
The two deletion callables continue to rely on verified Firebase Authentication context, explicit
payload validation and typed failure mapping; the D-66 alerts-only budget and billing cutoff
remain the development cost safety net.
...
- A future extension of App Check to Cloud Functions is a separate owner decision that MUST
  cover both deletion callables together.
