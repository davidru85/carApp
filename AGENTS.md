...
| `./gradlew testAndroidHostTest iosSimulatorArm64Test` with the four current D-75 `-x` paths from the complete command above | Common tests on both the JVM and Kotlin/Native. D-75 derives the exception from the transitive Firebase project graph and compares it with the declared paths. D-109 also compiles the composition-owned `IosLocaleProvider` source into `:shared` `iosTest`, so this route executes its Foundation behavior without a second Firebase Apple dependency path. |
