...
/**
 * `Cancelled -> CANCELLED`, `CredentialAlreadyInUse -> CREDENTIAL_IN_USE`,
 * `NetworkUnavailable -> NETWORK`, `UidWouldChange -> UID_WOULD_CHANGE`, everything else
 * -> `UNKNOWN`.
 */
fun AuthError.toConversionFailureReason(): ConversionFailureReason =
    when (this) {
...
/**
 * `RequiresRecentLogin -> REQUIRES_RECENT_LOGIN`, `AccountDeletionRemoteFailed -> REMOTE_FAILED`,
 * `NetworkUnavailable -> NETWORK`, everything else -> `UNKNOWN`.
 */
fun AuthError.toDeletionFailureReason(): DeletionFailureReason =
    when (this) {
